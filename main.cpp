...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//クライアント領域のサイズ
	const int32_t kClientWidth = 1290;
	const int32_t kClientHeight = 720;

	//ウィンドウサイズを表す構造体にクライアント領域を入れる
	RECT wrc = { 0, 0, kClientWidth, kClientHeight };

	//クライアント領域を元にじっさいのサイズにwrcを変更してもらう
	...
	//仕様するアダプタ酔うの変数。最初にnullptrを入れておく
	...
	DXGI_ERROR_NOT_FOUND; ++i) {
		...
		assert(SUCCEEDED(hr));	//取得できてないのは一大事
		//ソフトウェアアダプタでなければ採用！
	...
	//喜びの舞い
	Log(logStream, "Complate create D3D12Device!!\n"); // 初期化完了のログをだす
	...
	//コマンドアロケータの生成がうまくいかなかったので再帰できない
	assert(SUCCEEDED(hr));
	...
	swapChainDesc.Width = kClientWidth;	//画面の幅。 ウィンドウのクライアント領域を同じにしておく
	swapChainDesc.Height = kClientHeight;	//画面の高さ。 ウィンドウのクライアント領域を同じにしておく
	...
	//まず１つ目を作る。１つ目は最初の所に作る。作る場所をこちらで指定して上げる必要がある
	...
	inputElementDescs[0].SemanticName = "POSITIO";
	...
	graphicsPipelineStateDesc.VS = { vertexShaderBlob->GetBufferPointer(),
									vertexShaderBlob->GetBufferSize() };	//VertexShaderBlob
	graphicsPipelineStateDesc.PS = { pixelShaderBlob->GetBufferPointer(),
									pixelShaderBlob->GetBufferSize() };	//PixelShaderBlob
