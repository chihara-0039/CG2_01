...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	HWND hwnd = CreateWindow(wc.lpszClassName, // 利用するクラス名
		L"LE2C_21_チハラ_シゴウ", // タイトルバーの文字(何でもよい)
	...
	// ↓ ここで中身を確認
	std::cout << "OBJ 頂点数 : " << modelData.vertices.size() << std::endl;
	std::cout << "マテリアルのテクスチャパス : "
		<< modelData.material.textureFilePath << std::endl;

	if (modelData.vertices.empty()) {
		std::cerr << "[ERROR] OBJ から頂点が 1 つも読めていません\n";
	}

	if (modelData.material.textureFilePath.empty()) {
		std::cerr << "[WARN] MTL からテクスチャパスが取れていません\n";
	} else if (!std::filesystem::exists(modelData.material.textureFilePath)) {
		std::cerr << "[ERROR] テクスチャファイルが存在しません: "
			<< modelData.material.textureFilePath << std::endl;
	}
...
#pragma endregion
	...
	// 手前側の基準位置
	const float baseX = 0.0f;   // 一番手前の板ポリの X（好きに調整してOK）
	const float baseY = 0.0f;    // 高さは固定
	const float baseZ = 5.0f;    // 一番手前の板ポリの Z

	// 1枚ごとの間隔
	const float spacing = 0.5f;  // 「ナナメ方向」の見た目の間隔
	...
	const float stepX = (dirX / len) * spacing;
	const float stepZ = (dirZ / len) * spacing;

	for (uint32_t index = 0; index < kNumInstance; ++index) {

		transforms[index].scale = { 0.5f, 0.5f, 1.0f };
		...
		// インデックスごとにナナメ方向へ一定距離ずつ奥へ
		transforms[index].translate = {
			baseX + stepX * static_cast<float>(index), // X も増える
			baseY,
			baseZ + stepZ * static_cast<float>(index)  // Z も増える（奥へ）
		};
	}
	...
	directionalLightData->direction = Normalize({ 0.0f, 0.0f, 1.0f });
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			ImGui::SliderFloat3("Camera Pos", &cameraTransform.translate.x, -10.0f, 10.0f);
			ImGui::SliderAngle("Cam Rot X", &cameraTransform.rotate.x, -180.0f, 180.0f);
			...
			ImGui::SliderInt("Instance Index", &instanceIndex, 0, (int)kNumInstance - 1);

			// 範囲外にならないようにクランプ（念のため）
			if (instanceIndex < 0) { instanceIndex = 0; }
			if (instanceIndex >= (int)kNumInstance) { instanceIndex = (int)kNumInstance - 1; }

			// 選択中インスタンスの Transform を直接いじれるようにする
			ImGui::SliderFloat3("Inst Scale",
				&transforms[instanceIndex].scale.x, 0.1f, 5.0f);
			ImGui::SliderAngle("Inst Rot X",
				&transforms[instanceIndex].rotate.x, -180.0f, 180.0f);
			ImGui::SliderAngle("Inst Rot Y",
				&transforms[instanceIndex].rotate.y, -180.0f, 180.0f);
			ImGui::SliderAngle("Inst Rot Z",
				&transforms[instanceIndex].rotate.z, -180.0f, 180.0f);
			ImGui::SliderFloat3("Inst Translate",
				&transforms[instanceIndex].translate.x, -10.0f, 10.0f);

			// 位置をテキストでも確認
			ImGui::Text("Inst[%d] Pos = (%.2f, %.2f, %.2f)",
				instanceIndex,
				transforms[instanceIndex].translate.x,
				transforms[instanceIndex].translate.y,
				transforms[instanceIndex].translate.z);
			...
			// 単位行列を書き込んでおく04_00
			transformationMatrixDataSprite->WVP = worldMatrixSprite;
		   ...
		   /*commandList->SetGraphicsRootConstantBufferView(
			...
			commandList->DrawInstanced(
				UINT(modelData.vertices.size()), kNumInstance, 0, 0);
