...
def portfolio():
    ...
    new_page(d,"現在地と今後")
    heading(d,"現在できていること"); bullets(d,["BaseSceneを継承したタイトル／ゲーム／クリア各シーンとSceneManagerによる遷移","移動・ジャンプ・カメラ・落下復帰・スター取得判定","スター取得時の短い演出、CONGRATULATION表示、継続する花火演出","外部ステージの読込、天候、照明、GPUパーティクル、ポストエフェクト"])
