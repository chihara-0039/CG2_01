...
portfolio=[
('作品概要','移動・ジャンプ・カメラ操作で立体ステージを攻略し、星のゴールを目指す3D探索アクションです。',[('i',('frame_05.png',165*mm,'雷と照明が変化するステージ。星がゴールの目印。')),('h','コアループ'),('b',['ステージを観察し、移動とジャンプで足場を攻略する','カメラを回転・ズームして進行ルートを探す','星の座標へ到達してゴールする']),('h','操作'),('p','キーボード／マウスとXboxコントローラーに対応。プレイヤー追従カメラと全体カメラを切り替えられます。')]),
...
('現在地と今後','機能を増やす段階から、遊びへ結び付けて磨く段階へ進めています。',[('h','現在できていること'),('b',['移動・ジャンプ・カメラ・落下復帰・ゴール判定','外部ステージの読込とホットリロード','天候、照明、GPUパーティクル、ポストエフェクト','GPUスキニング、アニメーション補間、ボーンデバッグ']),('h','次に強化すること'),('b',['ステージ攻略のバリエーションとギミック追加','ゴールまでの誘導とクリア演出の完成度向上','操作感、当たり判定、カメラ挙動の継続調整','短時間で作品の魅力が伝わる企業向けデモ導線'])])]
...
('2. プレイヤーとカメラ','探索ゲームの操作性を支える基本機能です。',[('b',['移動・ジャンプ・落下時のスポーン／中継地点復帰','追従カメラと全体カメラの切替','マウスとXboxコントローラーによる回転・ズーム','ゴール座標への到達判定とシーン進行']),('i',('frame_05.png',160*mm,'ゲームプレイ画面。'))]),
...
('6. 描画と今後の改善','ゲームとエディタの安定性を継続して高めます。',[('b',['GPUパーティクルとエフェクトエディタ','Grayscale、Vignetting、Dissolve等のポストエフェクト','複数ライトの格納とシーン別リセット','D3D12リソースとディスクリプタの使用範囲を整理']),('h','今後'),('p','GPU負荷の計測、モデル切替の回帰テスト、ステージデータ検証、ゲーム側の完了定義に沿ったチェックを追加します。')])]
